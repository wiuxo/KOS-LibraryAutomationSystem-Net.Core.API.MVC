...
namespace KOS.Business.Auth.Handlers.Commands
{
    public class RemoveUserRole : IRequest<IResponse>
    {
        ...
        public class RemoveUserRoleHandler : IRequestHandler<RemoveUserRole, IResponse>
        {
            ...
            {
                var removeUserRole = await _userRoleRepository.GetAsync(x => x.UserID == request.UserId && x.RoleId == request.RoleId);
                if (removeUserRole == null) return new Response<UserRole>(null, false, "A user with this ID and role doesn't exist.");
