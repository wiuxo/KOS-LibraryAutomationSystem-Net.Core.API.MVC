...
namespace KOS.Business.Auth
{
    public class RegisterValidator : CustomValidator<RegisterUser>
    {
        ...
        {
            RuleFor(x => x.UserName).MaximumLength(16).WithMessage("Username must be 16 or less characters").NotEmpty().WithMessage(ValidatorMessage.IsNull);
            RuleFor(x => x.Password).Must(RegularEx.IsSafePassw).WithMessage(ValidatorMessage.IsSafePassw);
            RuleFor(x => x.FirstName).MaximumLength(16).WithMessage("Firstname must be 16 or less characters").NotEmpty().WithMessage(ValidatorMessage.IsNull);
            RuleFor(x => x.LastName).MaximumLength(16).WithMessage("Lastname must be 16 or less characters").NotEmpty().WithMessage(ValidatorMessage.IsNull);
        }
    }
    ...
    public class LoginValidator : CustomValidator<LoginUser>
    {
        ...
        {
            RuleFor(x => x.UserName).MaximumLength(16).WithMessage("Username must be less than 16 characters").NotEmpty().WithMessage(ValidatorMessage.IsNull);
            RuleFor(x => x.Password).Must(RegularEx.IsSafePassw).WithMessage(ValidatorMessage.IsSafePassw);
        }
    }
