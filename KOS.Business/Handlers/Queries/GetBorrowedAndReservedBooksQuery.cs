...
public class GetBorrowedAndReservedBooksQuery : IRequest<IResponse>
{
    ...
    public class GetBorrowedAndReservedBooksQueryHandler : IRequestHandler<GetBorrowedAndReservedBooksQuery, IResponse>
    {
        ...
        public async Task<IResponse> Handle(GetBorrowedAndReservedBooksQuery request,
            CancellationToken cancellationToken)
        {
            var books = await _bookRepository.GetListAsync(x =>
                x.HoldStatus != null && x.BorrowerID != null && x.IsRemoved != 1);
            if (!books.Any())
                return new Response<Book>(null, false, "No book in library that is both borrowed and reserved.");
