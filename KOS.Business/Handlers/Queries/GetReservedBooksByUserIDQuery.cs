...
public class GetReservedBooksByUserIdQuery : IRequest<IResponse>
{
    ...
    public class GetReservedBooksByUserIdHandler : IRequestHandler<GetReservedBooksByUserIdQuery, IResponse>
    {
        ...
        {
            var book = await _bookRepository.GetListAsync(x =>
                x.HoldStatus == request.UserID && x.IsRemoved != 1 && x.BorrowerID == null);
            return !book.Any()
                ? new Response<IEnumerable<Book>>(null, false, "There is no reserved book for this user.")
