...
public class GetBorrowedBooksWithNoHoldByIDQuery : IRequest<IResponse>
{
    ...
    public class
        GetBorrowedBooksWithNoHoldByIDQueryHandler : IRequestHandler<GetBorrowedBooksWithNoHoldByIDQuery, IResponse>
    {
        ...
        public async Task<IResponse> Handle(GetBorrowedBooksWithNoHoldByIDQuery request,
            CancellationToken cancellationToken)
        {
            var books = await _bookRepository.GetListAsync(x =>
                x.HoldStatus == null && x.BorrowerID == request.UserID && x.IsRemoved != 1);
